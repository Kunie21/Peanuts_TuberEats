...
{
	g_Model[MISSILE_TYPE_SALMON] = MODEL_SUSHI01;
	//g_Model[MISSILE_TYPE_01] = MODEL_MISSILE1;
	g_Model[MISSILE_TYPE_DONUTS] = MODEL_SUSHI01;
	g_Model[MISSILE_TYPE_CANDY] = MODEL_SUSHI01;
	g_Model[MISSILE_TYPE_RAMEN] = MODEL_SUSHI01;
	g_Model[MISSILE_TYPE_IKURA] = MODEL_SUSHI01;
	//g_Model[MISSILE_TYPE_DONUTS] = MODEL_DONUT01;
	//g_Model[MISSILE_TYPE_CANDY] = MODEL_LOLLIPOP01;
	//g_Model[MISSILE_TYPE_RAMEN] = MODEL_RAMEN;
...
void DrawMissile(void)
{
	...
	if (instCount > 0)
	{
		SetWorldBuffer(&XMMatrixIdentity());	// ワールドマトリックスの設定
		static MATERIAL material;
		DrawModelInstanced(&g_Model[MISSILE_TYPE_SALMON + GetPlayerRocket()], instCount, &material);	// モデル描画
	}
}
void DrawMissileHave(void)
{
	...
	for (int i = 0; i < GetPlayerMissiles(); i++)
	{
		...
		b_pInstance->scl[instCount] = { MISSILE_SCL, MISSILE_SCL, MISSILE_SCL, 0.0f };
		b_pInstance->rot[instCount] = { 0.0f, XM_PIDIV2, 0.0f, 0.0f };
		b_pInstance->pos[instCount] = { MISSILE_RADIUS * cosf(rot), ROCKET_Y + (MISSILE_RADIUS * sinf(rot)), 0.0f, 0.0f };
		b_pInstance->col[instCount] = { 1.0f, 1.0f, 1.0f, 1.0f };
	...
	{
		SetWorldBuffer(&XMMatrixIdentity());	// ワールドマトリックスの設定
		DrawModelInstanced(&g_Model[MISSILE_TYPE_SALMON + GetPlayerRocket()], instCount);	// モデル描画
	}
}
void DrawMissileFireHave(void)
{
	...
	{
		rot = g_Rot + (XM_2PI / GetPlayerMissiles()) * (float)i;
		scl = GetPlayerFireScale() * 0.3f;
		b_pInstance->scl[instCount] = {
			scl + (float)(rand() % 10) * 0.02f * scl,
			scl + (float)(rand() % 10) * 0.02f * scl,
			scl + (float)(rand() % 10) * 0.02f * scl,
			0.0f
		};
		b_pInstance->rot[instCount] = { XM_PI, 0.0f, 0.0f, 0.0f };
		b_pInstance->pos[instCount] = {
			MISSILE_RADIUS * cosf(rot),
			ROCKET_Y + (MISSILE_RADIUS * sinf(rot)),
			-15.0f, 0.0f
		};
		b_pInstance->col[instCount] = { 1.0f, 1.0f, 1.0f, 1.0f };
